...
web, which reports `preview`, and the staging API, which reports `staging`.
Production web and API both report `production`. Both base URLs must be clean
HTTPS origins: no credentials, query, hash, or path other than `/`. The command
