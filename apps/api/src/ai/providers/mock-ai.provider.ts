...
function buildRecipe(input: {
  ...
  return {
    ...
    steps: [
      {
        step: 1,
        what_to_do: 'Prepare ingredients and cook the rice.',
        duration_minutes: 15,
        temperature: null,
      ...
      {
        step: 2,
        what_to_do: 'Cook the protein with aromatics until done.',
        duration_minutes: 10,
        temperature: null,
        temperature_unit: null,
      ...
      {
        step: 3,
        what_to_do: 'Fold in vegetables and serve over rice.',
    ...
    meal_types: [input.mealStyle === 'meal_prep' ? 'lunch' : 'dinner'],
    difficulty: 'easy',
    difficulty_reason:
      'Mock recipe uses a short ingredient list and simple pan cooking.',
...
function normalizeRecipe(recipe: SwapIngredientDto['recipe']): AiRecipePreview {
  return {
    ...
    difficulty_reason: recipe.difficulty_reason ?? null,
