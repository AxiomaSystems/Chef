...
const LEGACY_LABELS: MealEventLabel[] = ['breakfast', 'lunch', 'dinner'];
...
function getRecipeScale(event: SharedMealEvent, recipe: BaseRecipe) {
  return event.servings / Math.max(recipe.servings, 1);
}
...
export class MealPlanService {
  ...
  async createCartFromPlan(
    ...
    actorUserId?: string,
  ): Promise<CreateMealPlanCartResponse> {
    const actor = await this.userContextService.resolveActorUser(actorUserId);
    ...
    const selections: CartSelection[] = plannedRecipeEvents.map((event) => ({
      ...
      quantity: 1,
      servings_override:
        event.recipe && event.servings !== event.recipe.servings
          ? event.servings
          : undefined,
    }));
    ...
    return {
      cart,
      selections,
      event_count: plannedRecipeEvents.length,
    };
  }
  ...
  private mapEvent(row: PrismaMealEvent): SharedMealEvent {
    return {
      ...
      meal_label: row.mealLabel as MealEventLabel,
      custom_label: row.customLabel,
      source_type: row.sourceType as MealEventSourceType,
      recipe_id: row.recipeId,
      title: row.title,
      servings: row.servings,
      status: row.status as MealEventStatus,
      locked: row.locked,
      notes: row.notes,
  ...
  private buildGrocerySummary(events: MealEventWithRecipe[]) {
    const groceryMap = new Map<string, AggregatedIngredient>();
    ...
    const items = Array.from(groceryMap.values()).sort((left, right) =>
  ...
  private mapEventsToLegacyWeek(
    ...
    for (const row of events) {
      if (!LEGACY_LABELS.includes(row.mealLabel as MealEventLabel)) {
        continue;
      }
