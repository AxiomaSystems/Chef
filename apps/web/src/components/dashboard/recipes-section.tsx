  ...
  return (
    ...
    >
      <div className="mb-4 flex items-center justify-between">
        <StatusPill
          ok={recipes.ok}
          label={recipes.ok ? "Connected" : "Issue"}
        />
        {!recipes.ok && recipes.error ? (
          <span className="text-sm text-[#ba1a1a]">{recipes.error}</span>
        ) : null}
      </div>
      ...
      <div className="grid gap-3 md:grid-cols-2">
        {recipes.data.slice(0, 6).map((recipe) => (
          <article
            key={recipe.id}
            className="rounded-[1.4rem] border border-[#c0dedf] bg-[#fff8ef]/65 p-4"
          >
            <div className="flex items-start justify-between gap-3">
              <div>
                <h3 className="font-sans font-bold text-2xl text-[#132326]">
                  {recipe.name}
                </h3>
              ...
              <span className="rounded-full bg-[#f4790d] px-3 py-1 text-[10px] font-semibold uppercase tracking-[0.18em] text-[#fff8ef]">
